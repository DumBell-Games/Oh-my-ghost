...
Fuego::Fuego() : Entity(EntityType::FUEGO)
{
	name.Create("fuego");
}
...
bool Fuego::Update(float dt)
{	
	fuego->Update();
	app->render->DrawTexture(texture.get(), position.x - 50, position.y - 120, &fuego->GetCurrentFrame());
...
void Fuego::OnCollision(PhysBody* physA, PhysBody* physB) {
	switch (physB->ctype)
	{
...
void Fuego::FuegoStartAnims() {
	...
	fuego = GetAnimation("fuego");
}

Animation* Fuego::GetAnimation(SString name)
{
	for (ListItem<Animation*>* item = fuegoAnims.start; item != nullptr; item = item->next)
	{
