...
Dolly::Dolly() : Entity(EntityType::DOLLY)
{
	name.Create("dolly");
}
...
bool Dolly::Start() {
	...
	currentAnim = dollyIdle;

	return true;
}

bool Dolly::PreUpdate()
{
	return true;
}

bool Dolly::Update(float dt)
{	
	currentAnim->Update();
...
void Dolly::OnCollision(PhysBody* physA, PhysBody* physB) {
	switch (physB->ctype)
	{
...
void Dolly::StartAnims() {
    ...
    dollyIdle = GetAnimation("dollyIdle");
}
	

Animation* Dolly::GetAnimation(SString name)
{
	for (ListItem<Animation*>* item = dollyAnims.start; item != nullptr; item = item->next)
	{
