...
Perras::Perras() : Entity(EntityType::PERRAS)
{
...
bool Perras::Update(float dt)
{	
	perras->Update();
	app->render->DrawTexture(texture.get(), position.x - 50, position.y - 120, &perras->GetCurrentFrame());
...
void Perras::OnCollision(PhysBody* physA, PhysBody* physB) {
	switch (physB->ctype)
	{
...
void Perras::PerrasStartAnims() {
	...
	perras = GetAnimation("perras");
}

Animation* Perras::GetAnimation(SString name)
{
